...
SAP ialah aplikasi dalaman berasaskan PHP yang dibina di atas IQS Framework. Projek ini menggunakan fungsi teras seperti login berpusat, kawalan akses mengikut kumpulan, konfigurasi runtime, audit aktiviti, template emel, manual pengguna dan sambungan pangkalan data yang dikawal dari UI.
...
- Framework updates must be reviewed manually against SAP project files before they are applied.
...
SAP uses three database access patterns inherited from IQS Framework:
...
3. Configure the Nginx virtual host.
   ...
   Set the document root to `/var/www/app/sap-uat/public`, route PHP requests to the installed PHP-FPM socket, disable directory listing, and deny access to hidden or sensitive files. Keep the Nginx configuration outside this repository under normal system administration controls.
...
This README documents the current SAP repository. Historical IQS Framework release details remain in `CHANGELOG.md`, while project-specific runtime and deployment guidance must use the SAP repository path.
