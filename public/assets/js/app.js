...
(function ($) {
    ...
    function initLeftSidebar() {
        ...
        if ($(".side-nav").length) {
            ...
            // Let Bootstrap handle open, but prevent close
            navToggle.on('click', function(e) {
                ...
                // If menu is open, prevent close (don't prevent default for open)
                if (isShown) {
                    e.preventDefault();
                    e.stopPropagation();
                    return false;
                ...
                // Menu is closed - let Bootstrap handle the open
                // Close other open menus first (accordion behavior)
                var parent = $target.parents('.collapse.show')[0];
                $('.side-nav .collapse.show').each(function() {
                    var el = this;
                    if (el !== $target[0] && el !== parent) {
                        if (typeof bootstrap !== 'undefined' && bootstrap.Collapse) {
                            var otherInstance = bootstrap.Collapse.getInstance(el);
                            if (otherInstance) {
                                otherInstance.hide();
                            }
                        }
                        var otherToggle = $(".side-nav li [data-bs-toggle='collapse'][href='#" + el.id + "']");
                        if (otherToggle.length) {
                            otherToggle.addClass('collapsed');
                            otherToggle.attr('aria-expanded', 'false');
                        }
                    }
                });
                
                // Don't prevent default - let Bootstrap handle the open
            });

            // No Bootstrap events needed - we handle everything manually
...
class ThemeCustomizer {
    ...
    setSwitchFromConfig() {
        ...
        try { sessionStorage.setItem('__CONFIG__', JSON.stringify(this.config)); } catch (e) {}
